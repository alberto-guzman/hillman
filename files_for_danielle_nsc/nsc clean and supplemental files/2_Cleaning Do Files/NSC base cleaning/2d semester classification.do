...
*******************************************************************************
	...
	********
	*Weird Start and End Dates - Only Counting First Semester as Enrolled Semester -- cannot determine whether student was enrolled the entire time
	********
	
	*Start Month - 8, 9, 10, 11, 12
	replace Semester = 2 if Semester==. & (StartMonth==8 | StartMonth==9 | StartMonth==10 | StartMonth==11 | StartMonth==12)
	
	*Start Month - 1, 2, 3, 4, 5
	replace Semester = 1 if Semester==. & (StartMonth==1 | StartMonth==2 | StartMonth==3 | StartMonth==4 | StartMonth==5)
	
	*Start Month - 6, 7
	replace Semester = 3 if Semester==. & (StartMonth==6 | StartMonth==7)
