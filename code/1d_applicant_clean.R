...
df_2017 <- read_excel(here("data", "hillman_2017.xlsx")) |>
  ...
  select(-c(
    ...
    first_reference_full_name, first_reference_email, sec_reference_full_name,
    sec_reference_email, computer_proficiency, lab_experimentation, housing,
    marketing, name, self_agree_statement, ...69, ...70, green_card
  )) |>
...
df_2018 <- read_excel(here("data", "hillman_2018.xlsx")) |>
  ...
  select(-c(
    ...
    "absences_(y/n)", "absences_(#_of_days)",
    "wet_v_dry", 
    ...
    submission_date,
    "if_yes,_please_explain", email, home_phone, cell_phone, address,
    ...
    "how_do_you_identify_yourself_[other]", "vi._housing"
  )) |>
  rename(
...
df_2019 <- read_excel(here("data", "hillman_2019.xlsx"), sheet = "All (Extra info)") |>
  ...
  select(-c(
    ...
    "parent_or_legal_guardian's_email...15", housing, "1st_choice", "2nd",
    "3rd", "4th", "5th", "6th", expected_absences, "#_of_absences",
    wet_or_dry_lab_preferred,
    "how_would_you_judge_your_sense_of_comfort_in_computer_use_and_applications_choose_one_of_the_following",
    "how_would_you_judge_your_sense_of_comfort_in_computer_use_and_applications_choose_one_of_the_following_[other]",
    "parent_or_legal_guardian's_full_name...40",
    "parent_or_legal_guardian's_email...41",
    "upload_your_essay...50", "a.", "b.", "upload_transcript",
    "would_you_like_to_upload_your_essay_or_use_the_space_provided",
    recommender, submission_date, "stage_completion_(%)",
    ...
    "upload_your_essay...79", "address...36", "city...37", "state...38",
    "zip...39", group, "prior_research_experiences.",
    list_science_and_math_courses_with_grades, "stipend_eligible"
  )) |>
  rename(
    high_school  = school,
    city         = "city...11",
...
df_2020 <- read_excel(here("data", "hillman_2020.xlsx")) |>
  ...
  select(
    ...
    "are_you_an_american_citizen", "stipend_eligible...77"
  ) |>
  rename(
    first_name           = first,
    last_name            = last,
...
df_2021 <- read_excel(here("data", "hillman_2021.xlsx")) |>
  ...
  select(
    ...
    city, state, zip, "are_you_a_current_jack_kent_cooke_foundation_jkcf_young_scholar",
    grades_gpa, grades_is_this_gpa_weighted, "psat_scores_math":"act_scores_science",
    ...
    "are_you_an_american_citizen", "stipend_eligible"
  ) |>
  rename(
    first_name           = first_1,
    last_name            = last_2,
...
# -----------------------------------------------------------------------------
# Merge All Years ------------------------------------------------------------
applicants <- bind_rows(df_2017, df_2018, df_2019, df_2020, df_2021, df_2022, df_2023)

# Remove columns not needed in the final analysis and clean up names
applicants <- applicants |>
  select(-c(
    high_school_pub_priv, school_district, reduced_lunch,
    offer_ap, `income_eligible_(reduced_lunch)`, alumni, act_verbal,
    low_income, middle_name
  )) |>
