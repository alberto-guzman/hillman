...
matched_all <- readRDS(here(
  "data",
  "matched",
  "matched_all_states_year_only.rds"
))
...
format_cell <- function(x, w, binary, digits = 3) {
  m <- weighted_mean(x, w)
  if (is.na(m)) {
    return("--")
  }
  ...
  if (binary) {
    ...
    sprintf("%.3f (%.3f)", round(m, digits), round(s, digits))
  }
}
    ...
    mutate(
      treated = map_chr(
        ...
        ~ {
          ...
          if (.x %in% school_pct_vars) {
            x <- x / 100
          }
          format_cell(
            x[trt_idx],
            data$weights[trt_idx],
            !(.x %in% continuous_vars)
          )
        }
      ),
      control = map_chr(
...
desc_pa <- build_descriptives(
  matched_pa,
  bind_rows(individual_vars, school_vars)
)
...
# Build a linesep vector that inserts \addlinespace[3pt] after the last row
# of each group and "" everywhere else — adds breathing room only at group
# transitions without padding every row.
group_sizes <- group_index(desc_combined$group)
linesep_t1 <- rep("", nrow(table1_df))
cumulative <- cumsum(group_sizes)
linesep_t1[cumulative[-length(cumulative)]] <- "\\addlinespace[3pt]"
...
table1_kbl <- table1_df |>
  kbl(
    ...
    col.names = c(
      ...
      sprintf("Matched Comparison ($n=%d$)", n_pa_c),
      sprintf("Treated ($n=%d$)", n_all_t),
      sprintf("Matched Comparison ($n=%d$)", n_all_c)
    ),
    caption = "Sample Descriptive Statistics for the Matched Treated and Comparison Groups",
    label = "descriptives",
    linesep = linesep_t1
  ) |>
  add_header_above(c(" " = 1, "PA" = 2, "All states" = 2)) |>
  pack_rows(
    index = group_index(desc_combined$group),
    italic = FALSE,
    bold = FALSE
  ) |>
  ...
  footnote(
    ...
    general = c(
      ...
      paste(
        ...
        "with replacement, caliper $=$ 0.25 SD of the propensity score.",
...
table2_kbl <- table2_df |>
  ...
  add_header_above(c(" " = 1, "PA" = 2, "All states" = 2)) |>
  pack_rows(
    index = group_index(table2_df$group),
    italic = FALSE,
    bold = FALSE
  ) |>
  kable_styling(latex_options = c("hold_position"), font_size = 10) |>
  footnote(
    ...
    general = c(
      ...
      paste(
        ...
        "is 1:3 nearest-neighbor with replacement, caliper $=$ 0.25 SD of the",
        "propensity score, with exact matching on application year for",
        "the PA sample and on application year $\\\\times$ PA residence",
        "(PA vs.\\\\ non-PA) for the all-states sample."
      )
    ),
    escape = FALSE,
    threeparttable = TRUE,
...
format_att_vert <- function(att, se, pval) {
  est <- ifelse(
    is.na(att),
    "--",
    paste0(formatC(att, digits = 3, format = "f"), stars(pval))
  )
  sed <- ifelse(
    is.na(se),
    "",
    paste0("(", formatC(se, digits = 3, format = "f"), ")")
  )
...
impact_pa <- build_impact_block(att_pa) |>
  rename_with(~ paste0("pa_", .), -c(panel, outcome, label))
impact_all <- build_impact_block(att_all) |>
  rename_with(~ paste0("all_", .), -c(panel, outcome, label))

impact_wide <- left_join(
  impact_pa,
  impact_all,
  by = c("panel", "outcome", "label")
) |>
  mutate(
  ...
  select(label, panel_full, pa_ctrl, pa_att, all_ctrl, all_att)
...
table3_kbl <- impact_wide |>
  ...
  add_header_above(c(" " = 1, "PA" = 2, "All states" = 2)) |>
  pack_rows(
    index = group_index(impact_wide$panel_full),
    italic = FALSE,
    bold = FALSE,
    hline_before = TRUE
  ) |>
  ...
  footnote(
    ...
    general = c(
      ...
      paste(
        sprintf(
          "\\\\textit{Notes:} PA sample: $n_t = %d$, $n_c = %d$. All-states sample: $n_t = %d$, $n_c = %d$.",
          n_pa_t,
          n_pa_c,
          n_all_t,
          n_all_c
...
render_one <- function(tex_path) {
  ...
  writeLines(
    c(
      "\\documentclass[10pt,border=8pt,varwidth=30cm]{standalone}",
  ...
  status <- system2(
    pdflatex,
    c("-interaction=batchmode", wrap_name),
    stdout = FALSE,
    stderr = FALSE
  )
  setwd(old_wd)
  ...
  if (status != 0 || !file.exists(pdf_path)) {
    message(
      ...
      basename(tex_path),
      " (see ",
      file.path(tmp, paste0(base, "_wrap.log")),
      ")"
    )
    return(invisible(NULL))
  }
