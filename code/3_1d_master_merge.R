...
treated_years <- treated_years %>%
  select(order(
    grepl("^treated_", names(.)),
    str_extract(names(.), "\\d{4}") %>% as.numeric()
  ))
...
merged_df <- merged_df %>%
  mutate(
    ...
    hawaiian_pacific_islander = if_else(
      str_detect(self_identity, "Hawaiian|Pacific"),
      1,
      0
    ),
  ...
  mutate(
    bi_multi_racial = if_else(
      rowSums(select(., african_american:white)) == 0,
      1,
      0
    ),
    racially_marginalized = if_else(
      african_american == 1 | hawaiian_pacific_islander == 1 | latinx == 1,
      1,
      0
    )
  )
...
merged_df$disability <- recode(
  merged_df$documented_disability,
  "Yes" = 1,
  "yes" = 1,
  "Disability" = 1,
  "No" = 0,
...
merged_df$neg_school <- if_else(
  str_detect(merged_df$school_impact, "yes|Yes"),
  1,
  0
)
...
merged_df$us_citizen <- if_else(
  str_detect(merged_df$american_citizen, "yes|Yes"),
  1,
  0
)
...
merged_df$stipend <- recode(
  merged_df$stipend,
  "Yes" = 1,
  "Stipend Eligible" = 1,
  "No" = 0,
  "Not Stipend Eligible" = 0
)
...
merged_df <- merged_df %>%
  mutate(
    urban = if_else(
      str_detect(geographic_location, regex("urban", ignore_case = TRUE)),
      1,
      0
    ),
    suburban = if_else(
      str_detect(geographic_location, regex("suburban", ignore_case = TRUE)),
      1,
      0
    ),
    rural = if_else(
      str_detect(
        geographic_location,
        regex("rural|small town", ignore_case = TRUE)
      ),
      1,
      0
    )
...
merged_df <- merged_df %>%
  mutate(
    african_american = if_else(
      str_detect(self_identity, regex("african|black", ignore_case = TRUE)),
      1,
      0
    ),
    asian = if_else(
      str_detect(self_identity, regex("asian", ignore_case = TRUE)),
      1,
      0
    ),
    hawaiian_pacific_islander = if_else(
      str_detect(self_identity, regex("hawaiian|pacific", ignore_case = TRUE)),
      1,
      0
    ),
    latinx = if_else(
      str_detect(self_identity, regex("hispanic|latino", ignore_case = TRUE)),
      1,
      0
    ),
    white = if_else(
      str_detect(self_identity, regex("caucasian|white", ignore_case = TRUE)),
      1,
      0
    ),
    middle_eastern = if_else(
      str_detect(self_identity, regex("middle eastern", ignore_case = TRUE)),
      1,
      0
    ),
    other_race = if_else(
      str_detect(self_identity, regex("other", ignore_case = TRUE)),
      1,
      0
    ),
    unknown_race = if_else(
      str_detect(self_identity, regex("do not wish", ignore_case = TRUE)),
      1,
      0
    )
  ) %>%
