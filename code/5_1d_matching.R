...
cols_to_fill <- c("gender", "grade", "gpa", "psat_math", "stipend", "house_size", 
                   "racially_marginalized", "bi_multi_racial", "urban", 
...
merged_df_pa <- merged_df_pa[!is.na(merged_df_pa$treated_in_year),]
merged_df_pa <- merged_df_pa[!is.na(merged_df_pa$pa_state_name_cw),]

#exact match by year
# Perform matching using the 'matchit' function with multiple covariates
m.out <- matchit(
  treated_in_year ~ gender + grade + gpa + psat_math + stipend + house_size + racially_marginalized +
  ...
  data = merged_df_pa, # Data source
  method = "nearest", # Matching method
  exact = ~ year + pa_state_name_cw , # Exact matching variables
