...
pacman::p_load(
  ...
  parallel, data.table, tictoc, batchtools, styler, tidyverse, magrittr, lubridate
)
...
# read in data 
df <- read_excel("hillman_2017.xlsx")
colnames(df) %<>% str_replace_all("\\s", "_") %<>% tolower()

colnames(df)
df <- select(df, -c(date_of_birth, email,home_phone,cell_phone,address_1,address_2,
...
# read in data 
df <- read_excel("hillman_2018.xlsx")
colnames(df) %<>% str_replace_all("\\s", "_") %<>% tolower()
colnames(df) %<>% str_replace_all(":", "")
colnames(df) %<>% str_replace_all("\\?", "")

colnames(df)
df <- select(df, -c(site,date_of_birth,wet_lab_eligible,alumnus,'absences_(y/n)','absences_(#_of_days)','wet_v_dry',
...
# read in data 
df <- read_excel("hillman_2019.xlsx", sheet = "All (Extra info)")
colnames(df) %<>% str_replace_all("\\s", "_") %<>% tolower()
colnames(df) %<>% str_replace_all(":", "")
colnames(df) %<>% str_replace_all("\\?", "")

colnames(df)
df <- select(df, -c(date_of_birth, email, home_phone, cell_phone, address...10,
...
df_stipend <- read_excel("hillman_raw.xlsx", 
                         sheet = "2019")
...
# read in data 
df <- read_excel("hillman_2020.xlsx")
colnames(df) %<>% str_replace_all("\\s", "_") %<>% tolower()
colnames(df) %<>% str_replace_all(":", "")
colnames(df) %<>% str_replace_all("\\?", "")

colnames(df)
df <- select(df, c(first, last, gender, current_grade...5, alumni, race...15,
...
# merge back dob 
dob_df <- read_excel("hillman_raw.xlsx", 
...
applicants <- df %>% select(-c(sat_verbal,sat_writing,sat_reading_writing,psat_verbal,psat_writing,psat_reading_writing,act_math,act_read,act_science,act_writing))



#age relative to the year they applied
applicants$year_dt <- lubridate::ymd(applicants$year, truncated = 2L)
applicants$age <- trunc((applicants$dob %--% applicants$year_dt) / years(1))



as.Date(paste0(format(as.character(applicants$year), '-01-01')))

hist(applicants$grade)
hist(applicants$gpa)
