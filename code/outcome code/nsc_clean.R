...
df <- read_csv(
  "nsc/CLEANED_502003_T209672.202110221010_DA.csv",
  col_types = cols(
    `Your Unique Identifier` = col_skip(),
    `Middle Initial` = col_skip(),
    `Name Suffix` = col_skip(),
    `Requester Return Field` = col_skip(),
    `Search Date` = col_skip(),
    `Enrollment Begin` = col_date(format = "%Y%m%d"),
...
nsc.working <-
  df |>
  mutate(
    Enrollment.Status = ifelse(
      is.na(Enrollment.Status) == TRUE,
      'unk',
      Enrollment.Status
    ),
    enroll_length = Enrollment.End - Enrollment.Begin,
    enr_any_fall = Enrollment.Begin >=
      as.Date(paste0(format(Enrollment.Begin, '%Y'), '-08-01'), '%Y-%m-%d') &
      format(Enrollment.Begin, '%m') < 11,
    ...
    enr_first_fall20_pt = enr_first_fall20 &
      Enrollment.Status == 'Q' |
      Enrollment.Status == 'H' |
      Enrollment.Status == 'L',
...
nsc.working <-
  nsc.working |>
  mutate(
    enr_any_spring = Enrollment.Begin >=
      as.Date(paste0(format(Enrollment.Begin, '%Y'), '-01-01'), '%Y-%m-%d') &
      format(Enrollment.Begin, '%m') < 8,
    enr_first_spring21 = format(Enrollment.Begin, '%Y') == 2021 &
      enr_any_spring,
    enr_any_20 = format(Enrollment.Begin, '%Y') == 2020 &
      enr_first_fall20 |
      enr_first_spring21,
...
nsc.working <-
  ...
  mutate(
    ...
    comp_pers_spring21 = pers_spring21 &
      Enrollment.End >= as.Date('2021-05-01'),
    pers_fall21 = format(Enrollment.Begin, '%Y') == 2021 &
      enr_any_fall &
      comp_pers_spring21,
    comp_pers_fall21 = pers_fall21 & Enrollment.End >= as.Date('2021-12-01'),
    pers_spring22 = format(Enrollment.Begin, '%Y') == 2022 &
      enr_any_spring &
      comp_pers_fall21,
...
nsc.working <-
  nsc.working |>
  mutate(
    gap_sem = !enr_first_fall20 &
      format(Enrollment.Begin, '%Y') == 2021 &
      enr_any_spring,
    gap_year = !enr_first_spring21 &
      gap_sem &
      format(Enrollment.Begin, '%Y') == 2021 &
      enr_any_fall
  )
...
#replace NA to False
nsc.working <- nsc.working %>%
  mutate_each(funs(replace(., is.na(.), F)), enroll_length:gap_year)
#convert boolean to integers
nsc.working <- nsc.working %>%
  mutate_each(funs(as.integer(.)), enroll_length:gap_year)


#create one line per row
test <- nsc.working |>
  group_by(applicant_id) |>
  summarise(enroll_fall = max(gap_sem), tx = max(sample_intervention))
