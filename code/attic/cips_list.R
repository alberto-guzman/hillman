...
df <- read_csv(here("data", "raw", "cips", "stemList2022.txt"))
...
DHSSTEMCIPS <- str_replace_all(
  string = DHSSTEMCIPS$cip_code,
  pattern = '\\.',
  ''
)
...
curated_cips <- read_csv(here("data", "raw", "cips", "ManualCurationMajors.csv"))
